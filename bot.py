...
Формат вывода — МАКСИМУМ 5 строк ВСЕГО (включая заголовок). Никакой развёрнутой прозы и объяснений.

Структура:
Строка 1 (заголовок): ✨ Астро-день, {день недели} {число} {месяц}
Строка 2: Фон дня — одна короткая фраза: общий характер + смена периода с временем если есть.
Строка 3: Главная рекомендация — одна фраза. Что делать / чего избегать сегодня.
Строки 4–5 (только если есть встречи из календаря): каждая встреча — одна строка: "📅 [название] — зелёный свет" или "📅 [название] ⚠️ осторожно". Без объяснений почему.

Правила:
- Если день ровный — строки 3–5 можно опустить.
- Если острый день (💥 КОНФЛИКТ, ❌ РАЗРУШЕНИЕ) — строка 2 начинается с ⚠️.
- НЕ объяснять астрологические периоды. Только конкретный вывод.
- Не писать "потому что", не раскрывать названия энергий подробно — только эмодзи периода.
...
def build_astro_block(astro_events: list[dict], meetings: list[dict], today_display: str) -> str:
    ...
    try:
        ...
        text = ask_claude(ASTRO_SYSTEM_PROMPT, payload, model=MODEL_SMART, max_tokens=200)
        return text.strip()
    except Exception as e:
...
async def prepare_news_digest(bot: Bot) -> None:
    ...
    tg_lines = [f"📰 НОВОСТИ НА СЕГОДНЯ · {_format_date_ru(now)}"]
