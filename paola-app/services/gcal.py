...
# Сравнение — ТОЧНОЕ по title.strip().upper(), поэтому значения в верхнем регистре
# и совпадают только с голым названием, не с «BBR: что-то».
# «Yulia a Monza» — технические встречи присутствия (новое имя).
# Голое «BBR» — старые блоки присутствия на весь день (Юлия переименовывает их в
# «Yulia a Monza»); их не биллим. А реальная PM-работа пишется как «BBR: описание»
# — точное совпадение её не ловит, и она проходит как Progett.
GCAL_SKIP_TITLES = {"YULIA A MONZA", "BBR"}
